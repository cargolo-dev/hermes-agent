...
MODE_KEYWORDS = {
    ...
    "sea": ("ocean", "sea", "seefracht", "bl", "bill of lading", "port", "vessel"),
...
def _normalize_asr_mode(value: Any) -> str:
    """Normalize transport mode to the ASR-internal mode vocabulary."""
    ...
    if mode in {"rail", "train"}:
        return "rail"
    # Generic ASR and non-ASR modes are not concrete enough for case-state mode.
    return "unknown"


def _tms_mode_from_snapshot(tms_snapshot: dict[str, Any] | None) -> str:
    detail = tms_snapshot.get("detail") if isinstance(tms_snapshot, dict) else {}
    ...
    for candidate in (
        ...
        detail.get("network"),
        tms_snapshot.get("transport_mode") if isinstance(tms_snapshot, dict) else None,
        tms_snapshot.get("network") if isinstance(tms_snapshot, dict) else None,
    ):
        mode = _normalize_asr_mode(candidate)
        if mode != "unknown":
            return mode
    return "unknown"
...
def bootstrap_case(
    ...
    *,
    ...
    tms_detail = tms_snapshot.get("detail") if isinstance(tms_snapshot, dict) else {}
    tms_mode = _tms_mode_from_snapshot(tms_snapshot)
    state.mode = tms_mode if tms_mode != "unknown" else _normalize_asr_mode(state.mode)
...
def process_email_event(
    ...
    *,
    ...
    if tms_mode != "unknown":
        state.mode = tms_mode
    elif entities_after.transport_mode_candidates:
        state.mode = _normalize_asr_mode(entities_after.transport_mode_candidates[0])
    else:
        state.mode = detect_mode(_body_blob(message))
    ...
    if tms_snapshot.get("source") == "live" and tms_snapshot.get("detail"):
        ...
        # Prefer concrete TMS transport mode/network over weak mail signature hints.
        tms_mode = _tms_mode_from_snapshot(tms_snapshot)
        if tms_mode != "unknown":
            state.mode = tms_mode
